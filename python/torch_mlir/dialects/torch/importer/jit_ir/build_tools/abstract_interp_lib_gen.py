...
@check_dtype_function(
    [Invocation(TensorOfShape(1, device="cpu"), 1.0, 0, dtype=torch.qint8),
    Invocation(TensorOfShape(1, device="cpu"), 1.0, 0, dtype=torch.quint8),
    Invocation(TensorOfShape(1, device="cpu"), 1.0, 0, dtype=torch.qint32)]
)
def aten〇quantize_per_tensor〡dtype(self_rank_dtype: Tuple[int, int], scale: float, zero_point: int, dtype: int) -> int:
    return dtype

@check_dtype_function(
    [Invocation(NonZeroDTensorWithDtype(torch.qint8, device="cpu")),
    Invocation(NonZeroDTensorWithDtype(torch.quint8, device="cpu")),
    Invocation(NonZeroDTensorWithDtype(torch.qint32, device="cpu"))]
)
def aten〇int_repr〡dtype(self_rank_dtype: Tuple[int, int]) -> int:
    if self_rank_dtype[1] == torch.quint8:
        return torch.uint8
    elif self_rank_dtype[1] == torch.qint8:
        return torch.int8
    elif self_rank_dtype[1] == torch.qint32:
        return torch.int32
