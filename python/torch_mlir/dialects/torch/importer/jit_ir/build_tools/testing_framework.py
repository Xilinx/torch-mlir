...
class Invocation:
    ...
    def to_real_op_args(self):
        ...
        def tensor_transformer(o):
            if o.dtype in [torch.qint8, torch.quint8, torch.qint32]:
                return torch.quantize_per_tensor(torch.ones(o.shape).to(o.device), 1.0, 0, o.dtype)
            return torch.ones(o.shape, dtype=o.dtype).to(o.device)
