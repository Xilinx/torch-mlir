...
OpFoldResult AtenToDtypeOp::fold(FoldAdaptor adaptor) {
  ...
  if (inputType != resType) {
    if (auto otherCast = getSelf().getDefiningOp<AtenToDtypeOp>()) {
      if (otherCast.getSelf().getType().cast<BaseTensorType>() == resType) {
...
void AtenToDtypeOp::getCanonicalizationPatterns(RewritePatternSet &patterns,
  ...
  patterns.add(+[](AtenToDtypeOp op, PatternRewriter &rewriter) {
    auto isSupported = [](AtenToDtypeOp op) {
      bool nonBlocking, copyArg;
      // The non_blocking arg must be `False`.
      if (!matchPattern(op.getNonBlocking(),
    ...
    auto otherCast = op.getSelf().getDefiningOp<AtenToDtypeOp>();
    if (!otherCast || !isSupported(otherCast))
      return failure();

    rewriter.replaceOpWithNewOp<AtenToDtypeOp>(
        op, op->getResult(0).getType(), otherCast.getSelf(), op.getDtype(),
...
OpFoldResult AtenDetachOp::fold(FoldAdaptor operands) {
  return getOperand();
}
