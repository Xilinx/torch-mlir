...
public:
  ...
  matchAndRewrite(AtenOpT op, OpAdaptor adaptor,
    ...
    if (rhsType.getElementType() != rhsAlphaMulElemType) {
      ...
      rhs = tosa::tosaCastTensorToType(rewriter, rhs, rhsType).value();
    }
...
LogicalResult ConvertAtenOp<AtenSliceTensorOp>::matchAndRewrite(
  ...
  auto slice = rewriter.create<tosa::SliceOp>(
      ...
      reshaped, tosa::getTosaConstShape(rewriter, op->getLoc(), startSlice),
      tosa::getTosaConstShape(rewriter, op->getLoc(), sliceShape));
